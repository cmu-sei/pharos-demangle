...
is in minimal machine-readable JSON with minimal spaces and no
newlines, and should be piped through a pretty printer (such as
B<python -m json.tool>) in order to be read by a human.
